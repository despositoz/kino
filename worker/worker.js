...
2. Лучшие варианты: необычное соотношение бюджета и мировых сборов; другой известный фильм того же режиссёра; возраст фильма, хронометраж, страна производства или принадлежность к коллекции. Выбери ровно один вариант и не добавляй второй факт рядом.
...
5. Напиши ровно одно короткое предложение на русском языке. В нём должно быть одно утверждение; не добавляй второе наблюдение, пример или справку. Не начинай с «А ты знал»: эта фраза уже стоит в заголовке карточки.
...
8. Если передан блок <previousInsights>, по возможности выбери другой факт и не повторяй прежние формулировки. Не выдумывай новый факт только ради отличия.
...
function feedPrompt(stats, catalog, previousInsights) {
  const previous = (Array.isArray(previousInsights) ? previousInsights : [previousInsights])
    .slice(0, 6)
    .map((insight) => compactText(insight, 600))
    .filter(Boolean);
  return [
    `<stats>\n${JSON.stringify(stats)}\n</stats>`,
    `<catalog>\n${JSON.stringify(catalog)}\n</catalog>`,
    previous.length ? `<previousInsights>\n${JSON.stringify(previous)}\n</previousInsights>` : "",
  ].filter(Boolean).join("\n");
}

function singleFeedSentence(value) {
...
export default {
  async fetch(request, env) {
    ...
    if (route === "review") {
      ...
      prompt = feedPrompt(stats, catalog, input.previousInsights || input.previousInsight);
      resultKey = "insight";
      maxOutputTokens = 110;
    }
    ...
    let text = (data.candidates?.[0]?.content?.parts || [])
      ...
      .join("")
      .trim();
    if (route === "feed") text = singleFeedSentence(text);
    if (!text)
